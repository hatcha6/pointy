...
backend-run-remote: backend-env backend-install ## Run Django against the remote relay (used by dev-remote; self-enrolls on first run).
	POINTY_RELAY_CONTROL_URL="https://$(RELAY_REMOTE_HOST)" \
		POINTY_RELAY_PUBLIC_API_URL="https://$(RELAY_REMOTE_HOST)" \
		POINTY_RELAY_CONNECTOR_ADDR="$(RELAY_REMOTE_CONNECTOR_DIAL)" \
		POINTY_RELAY_CONNECTOR_TLS_SERVER_NAME="$(RELAY_REMOTE_HOST)" \
...
frontend-integrations-preview: frontend-install ## Run the Shop Settings integrations UI preview harness as a local web server.
	cd "$(FRONTEND_DIR)" && $(FLUTTER) run -d web-server --web-hostname $(WEB_HOST) --web-port $(WEB_PORT) -t lib/dev/integrations_preview.dart

frontend-recharge-preview: frontend-install ## Run the POS top-up (recharge) UI preview harness as a local web server.
	cd "$(FRONTEND_DIR)" && $(FLUTTER) run -d web-server --web-hostname $(WEB_HOST) --web-port $(WEB_PORT) -t lib/dev/recharge_preview.dart

frontend-portal-payments-preview: frontend-install ## Run the "LNET top-ups done on the website" UI preview harness as a local web server.
	cd "$(FRONTEND_DIR)" && $(FLUTTER) run -d web-server --web-hostname $(WEB_HOST) --web-port $(WEB_PORT) -t lib/dev/portal_payments_preview.dart

frontend-printers-preview: frontend-install ## Run the device printers (jobs + editor) UI preview harness as a local web server.
	cd "$(FRONTEND_DIR)" && $(FLUTTER) run -d web-server --web-hostname $(WEB_HOST) --web-port $(WEB_PORT) -t lib/dev/printers_preview.dart
...
frontend-invoice-attribution-preview: frontend-install ## Run the invoice cashier/drawer-session attribution UI preview harness as a local web server.
	cd "$(FRONTEND_DIR)" && $(FLUTTER) run -d web-server --web-hostname $(WEB_HOST) --web-port $(WEB_PORT) -t lib/dev/invoice_attribution_preview.dart
