...
class AnalyticsEngine {
  ...
  /// [maxFlushBackoff]. Without this the engine has no failure state at all:
  /// see [track], where a queue that cannot drain turns the "batch is full"
  /// trigger into one HTTP request per event, forever.
  final Duration initialFlushBackoff;
  final Duration maxFlushBackoff;

  /// How many ingest requests may be sent per [flushInterval], across every
  /// flush in that window.
  ///
  /// A flush used to drain the whole queue in back-to-back chunks; on
  /// 2026-08-17 a till doing that took accepted ingest from 5 to 2,123 requests
  /// a minute, and for four minutes the shop's own traffic queued behind it —
  /// product-list took 104 seconds and the till could not sell.
  ///
  /// Deliberately a window rather than a per-flush cap. A queue that cannot
  /// drain sits above [maxBatchSize], so `track` calls `flush` again on the
  /// very next event: a per-flush cap would just hand each of those a fresh
  /// budget and bound nothing.
  ///
  /// Sized so only a backlog ever meets it. At the default batch size that is
  /// 600 events per two minutes — well above a busy till, which produces a few
  /// hundred — while a 2,000-event backlog spreads over about four windows
  /// instead of arriving as twenty simultaneous requests.
  ...
  /// on every API request (see PosApiSession.describeClient) so the backend can
  /// name the device behind a request it rejected.
  ...
  Future<void> _start() async {
    ...
    await trackUsage(
      ...
      metrics: {
        ...
        // "the tills take forever to open" had no number attached to it.
  ...
  /// is queued forever. Left on, the queue saturates and (before the backoff
  /// below) each new event fired its own doomed request. A device that
  /// structurally cannot deliver telemetry should not gather it.
  ...
  void setCurrentScreen(String? screenName) {
    ...
    // Close the open frame window first, billed to the outgoing screen. A batch carries whichever screen was
    // current when it was *emitted*, so without this a 10-second window that
    // spans a navigation is billed entirely to the screen the user landed on —
    // which quietly charges every destination for the cost of arriving at it.
  ...
  Future<void> track(
    ...
    try {
      ...
      // drain sits permanently above maxBatchSize, so without this guard the
      // "batch is full" trigger fires on EVERY event and the batching guarantee
      // inverts into one request per event — 5.1M of them in the field, at a
      // flat rate around the clock, with no backoff and a persisted queue that
      // survived restarts. While a flush is failing, only the timer retries,
      // and only once the backoff has expired.
  ...
  /// The timer's retry, which respects the backoff. [flush] itself does not:
  /// an explicit caller (the post-login backlog, teardown) has new information
  /// and should not be made to wait behind a failure that predates it.
  ...
  Future<void> flush() async {
    try {
      ...
      // are deleted per batch below.
      await flushPendingWrites();
      // Ship what is collected in chunks (the backend caps each request),
      // bounded per flush so a large backlog is paced across ticks rather than
      // fired at the server all at once; stop at the first failure so a down
      // backend gets one attempt per cycle, not a hammering.
      ...
      while (_queue.isNotEmpty) {
        ...
        // Delete what was delivered now rather than at the end: a crash between
        // here and the next batch would otherwise resend it. (A resend is safe
        // — the backend deduplicates on clientEventId — but pointless.)
        _pendingRemovals.addAll(submittedIds);
        await flushPendingWrites();
      }
      if (!madeProgress && _hasPendingWrites) {
        await flushPendingWrites();
      }
    } on Exception {
      return;
