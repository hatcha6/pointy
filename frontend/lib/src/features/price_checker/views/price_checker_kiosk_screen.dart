...
/// The live customer-facing kiosk: wires the barcode scanner and the LAN lookup
/// endpoint to [PriceCheckerKioskView], auto-resets between shoppers, and gates
/// the exit behind the configured PIN.
///
...
class _PriceCheckerKioskScreenState extends State<PriceCheckerKioskScreen> {
  static const _foundDwell = Duration(seconds: 12);
  ...
  Future<void> _onBarcode(String code) async {
    ...
    switch (result) {
      case Ok<PriceLookupResult>(value: final lookup):
        ...
        _scheduleReset(
          lookup.found ? _foundDwell : _notFoundDwell,
        );
