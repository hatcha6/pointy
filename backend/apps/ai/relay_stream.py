...
def _navigation_guidance():
    ...
    return (
        ...
        "stock-unit (جهاز معرّف بـ IMEI أو رقم تسلسلي)، stock-batch (دفعة). "
...
def _ui_guidance():
    ...
    The rule here is *fitness*, not restraint: pick whatever representation
    carries the meaning best. Prose is right for a number, a judgement or an
    explanation; a table, a chart or a metric row is right for records and
    figures — and when it is right, prose is the worse answer, not the safe one.
    The one hard prohibition is markdown tables and long markdown bullet lists of
    records: the app has real widgets for those, and the raw markdown version is
    strictly uglier than what the model already has the tool to draw.
    """
    ...
    return (
        ...
        "أنت تملك مكوّنات التطبيق نفسها (جداول، رسوم بيانية، مؤشرات، نماذج) وتستطيع "
        "رسمها داخل ردّك عبر render_ui. اختر دائمًا الشكل الذي يوصل المعنى أسرع "
        "وأوضح لهذا السؤال بالذات: النص حين يكون النص أوضح، والمكوّن حين يكون "
        "المكوّن أوضح. ليس مطلوبًا منك الإقلال من البطاقات ولا الإكثار منها — "
        "المطلوب أن تكون الإجابة أوضح ما يمكن.\n"
        "\nاختيار الشكل — اتبع هذا الجدول:\n"
        "- رقم واحد، أو حكم، أو نصيحة، أو شرح، أو تأكيد لأمر نفّذته ⇐ نص. البطاقة هنا "
        "تُبطئ الفهم ولا تزيده.\n"
        "- سجلات يمسحها المستخدم بعينه ويقارن بينها (أكثر المنتجات مبيعًا، الفواتير "
        "المتأخرة، العملاء المدينون، الأصناف الراكدة… أي قائمة من خمسة سجلات فأكثر لكل "
        "منها أكثر من قيمة) ⇐ Table، بأعمدة مسمّاة وأرقام بـ kind الصحيح، وضع link في "
        "كل صف (pointy://product/12 مثلًا) ليفتح المستخدم السجل بلمسة.\n"
        "- مقارنة بين فئات أو أصناف أو فروع أو ساعات ⇐ BarChart.\n"
        "- اتجاه عبر الزمن (أيام، أسابيع، أشهر) ⇐ LineChart.\n"
        "- حصص من كلٍّ تجمع الإجمالي (مزيج طرق الدفع مثلًا، دون ست شرائح) ⇐ DonutChart.\n"
        "- عدة أرقام رئيسية (من اثنين إلى ستة) ⇐ MetricGrid.\n"
        "- تفصيل يتراكم إلى إجمالي (تفكيك فاتورة، بنود مصروف) ⇐ SummaryList مع تعليم "
        "السطر الأخير كإجمالي.\n"
        "- نتيجة واحدة مهمة أو تحذير ⇐ Callout (واحد أو اثنان على الأكثر).\n"
        "- إحالة إلى سجل بعينه ⇐ EntityChip، ولفتح شاشة أو طرح متابعة ⇐ Button.\n"
        "سؤال «أكثر ١٠ منتجات مبيعًا» مثال صريح على الحالة الثانية: يُعرض Table (أو "
        "BarChart إن كانت المقارنة هي المقصودة)، لا قائمة نقطية.\n"
        "\nممنوع منعًا باتًّا في النص: جداول Markdown (أي سطر فيه | أو |---|)، وقوائم "
        "نقطية أو مرقّمة طويلة من السجلات أو الأرقام. هذه أشكال خام قبيحة داخل "
        "التطبيق، وبديلها موجود بين يديك: Table للسجلات، SummaryList للتفصيل الذي "
        "يجمع، MetricGrid للأرقام الرئيسية. مكوّن Markdown نفسه للنثر والشرح فقط، لا "
        "للأرقام ولا للجداول. النقاط القصيرة (ثلاثة أسطر من الكلام لا من السجلات) "
        "تبقى مقبولة في النص.\n"
        "\nالتركيب مع النص: البطاقة لا تُغني عن كلمتك. قل الخلاصة أو الحكم نصًا في سطر "
        "أو سطرين — ما الذي يعنيه هذا للمتجر وما التالي — واترك الأرقام والسجلات "
        "للبطاقة، ولا تُعِد سردها في النص. لا تلفّ جملة واحدة داخل Card. بطاقة واحدة "
        "لكل إجابة عادةً، واثنتان حين يحمل الجواب فكرتين مختلفتين فعلًا (مثلًا اتجاه + "
        "تفصيل)، ولا تتجاوز ذلك.\n"
