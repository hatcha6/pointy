...
def _action_guidance(shop):
    ...
    return (
        ...
        "(2) لكل بند فيه issues (نقص الكمية أو التكلفة) اسأل المستخدم القيمة الناقصة عبر "
        "ask_user (number) قبل إدراجه. "
        "(3) للبنود المطابقة (matched=true) استخدم variant_id مع الكمية والتكلفة من الفاتورة "
        "دون إنشاء منتج جديد. للبنود غير المطابقة اسأل المستخدم بسؤال product_picker (ضع في "
        ...
        "suppliers؛ وإن لم يُستخرج اسم مورّد فاسأل المستخدم عنه قبل المتابعة. "
        "(5) أنشئ أمر الشراء عبر create_resource على purchase-orders ببنود "
        "[{variant, quantity, unit_cost}] بالكميات والتكاليف الصحيحة تمامًا كما في الفاتورة. "
