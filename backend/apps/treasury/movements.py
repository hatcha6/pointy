...
def _transfer_rows(account, *, start, end):
    transfers = money_period(
        MoneyTransfer.objects.select_related("from_account", "to_account").filter(
            _transfer_touches(account)
        ),
        start,
        end,
    )
...
def _expense_rows(methods, *, start, end):
    expenses = money_period(
        Expense.objects.filter(payment_method__in=methods).select_related("category"),
        start,
        end,
    )
