...
dropped Linux capabilities, no privilege escalation, per-service memory and CPU
caps, and no database port exposed to the LAN. Login, admin-setup and
password-change endpoints are rate limited, and completed sales are append-only —
corrections go through the audited void/return flow rather than an edit.

Keep `.env` readable only by root (the installer enforces this): it holds the
database password and the Django secret key.
